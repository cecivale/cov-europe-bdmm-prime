...
args$input = "/Users/maceci/traj_10_test/notimeparameter/210122_europe10a.TL.traj"
...
ggplot() + geom_sf(data = worldmap, aes(fill = deme), alpha = 0.9, size = 0) +
...
case_data <- case_data %>%
  group_by(deme) %>%
  mutate(rollavg3 = zoo::rollmean(value, k = 3, fill = NA),
         #rollavg7 = zoo::rollmean(value, k = 7, fill = NA),
         datereport = date,
         date = date - 10)
...
case_data_pop <- case_data %>% left_join(population, by="deme") %>% mutate(ECDCcas100 = cumvalue * 1e5 / (pop * 1e3)) 
```
...
knitr::kable(case_data_pop %>% ungroup %>% filter(date == MRS + 10) %>% select(deme, cumvalue, pop, ECDCcas100) %>% mutate_if(is.numeric, function (x) round(x,1)), booktabs = TRUE, caption = "Total number of cases reported to ECDC 18th March")
```
::::
...
ggplot(case_data) +
  geom_line(aes(x = date, y = cumvalue, color = deme)) +
  geom_point(aes(x = date, y = cumvalue, color = deme)) +
...
min_date <- min(df_traj_cases$date)
df_traj_cases_complete <- df_traj_cases %>%
...
reported1 <- df_traj_cases %>%
  filter(var == "in_pop", date == MRS) %>%
  left_join(case_data_pop %>% rename(cumcases = cumvalue) %>% select(deme, date, cumcases, pop), by = c("deme", "date")) %>%
  group_by(var, deme, cumcases, pop) %>%
  summarise(median_in = median(cumvalue),
            l95_in = HDInterval::hdi(cumvalue, 0.95)[1],
            h95_in = HDInterval::hdi(cumvalue, 0.95)[2]) %>%
  mutate(times_med = median_in/cumcases,
         times_l = l95_in/cumcases,
         times_h = h95_in/cumcases,
         p_med = median_in/(pop*1e3) * 100,
         p_l = l95_in/(pop*1e3) * 100,
         p_h = h95_in/(pop*1e3) * 100)

reported2 <- df_traj_cases %>%
  filter(var == "in_pop") %>%
  left_join(case_data_pop %>% rename(cases = value, ecdcavg3=rollavg3) %>% select(deme, date, cases, pop, ecdcavg3), by = c("deme", "date")) %>%
  group_by(var, deme, date, cases, ecdcavg3) %>%
  summarise(median_in = median(value),
            l95_in = HDInterval::hdi(value, 0.95)[1],
            h95_in = HDInterval::hdi(value, 0.95)[2]) %>%
  mutate(report_rate = cases / median_in)

reported3 <- df_traj_cases %>%
  filter(var == "in_pop") %>%
  left_join(case_data_pop %>% rename(cases = value, ecdcavg3=rollavg3) %>% select(deme, date, cases, pop, ecdcavg3), by = c("deme", "date")) %>%
  group_by(var, deme, week = week(date), cases, ecdcavg3) %>%
  summarise(median_in = median(value),
            l95_in = HDInterval::hdi(value, 0.95)[1],
            h95_in = HDInterval::hdi(value, 0.95)[2]) %>%
  mutate(report_rate = cases / median_in)

reported3 <- df_traj_cases_complete %>%
  group_by(var, deme, week = week(date), cases, ecdcavg3) %>%
  summarise(median_in = median(value),
            l95_in = HDInterval::hdi(value, 0.95)[1],
            h95_in = HDInterval::hdi(value, 0.95)[2]) %>%
...
ggplot(reported3 %>% filter(deme != "China")) +
  geom_line(aes(week, report_rate, color = deme)) +
  geom_point(aes(week, report_rate, color = deme))
...
df_traj_cases %>%
    ...
    scale_color_manual(name = "",
                       ...
                       #values = c("#4ab9ed", dark_dcolors[[d]])) +
    scale_alpha_manual(name = "", values = c(0.2, 1), guide = FALSE) +
    scale_linetype_manual(name = "", values = c(1, 2),
                          labels = c(paste0(d, " inferred\n population trajectories"),
                                     paste0(ifelse(d == china_deme, "China", as.character(d)),
