...
case_data <- get_cases(demes, from = ymd("2019-09-01"), to = MRS + 10) %>%
  ...
  distinct_at(vars(deme, date, cumcases, cumdeaths), .keep_all = TRUE)
...
events[events$event == "O", "src"] = 2
events[events$event == "O", "mult"] =  1

df_cases1 <- events %>%
  group_by_at(vars(-mult)) %>%
  summarise(N = sum(mult), .groups = "drop") %>%
  mutate(event2 = ifelse(event == "M", paste0("IM_", src), NA),
         event3 = ifelse(event == "M", paste0("OM_", dest), NA)) %>%
  pivot_longer(src:dest, names_to = "role", values_to = "type") %>%
  mutate(event = ifelse(event == "M" & role == "src", event3,
                        ifelse(event == "M" & role == "dest", event2, event))) %>% 
  select(-c("role", "event2", "event3")) %>%
  filter(!is.na(type)) %>%
  group_by_at(vars(-N)) %>%
  summarise(value = sum(N), .groups = "drop") %>%
  pivot_wider(names_from = event, values_from = value, values_fill = list(value = 0)) %>%
  mutate(IM = IM_0 + IM_1 + IM_2 + IM_3 + IM_4 + IM_5,
         OM = OM_0 + OM_1 + OM_2 + OM_3 + OM_4 + OM_5) %>%
  group_by(traj, type) %>%
  arrange(traj, type, time) %>%
  mutate_at(tail(names(.), -4), list(c = cumsum)) %>%
  mutate(activePop = O_c + B_c + IM_c - D_c - OM_c - S_c,
         cumPop = O_c + B_c + IM_c,
         inPop = O + B + IM,
         outPop = D + OM + S) %>% 
  pivot_longer(O:outPop, names_to = "var", values_to = "value")
  # pivot_longer(O:outPop, names_to = c("value", "set"),
  #  names_pattern = "(.*)_(.)")
...
df_cases <- events %>%
  ...
  mutate(event = ifelse(event == "M" & role == "src", "OM",
         ...
         partner = ifelse(event == "OM", destmp,
                        ifelse(event == "IM", srctmp, NA))) %>% 
  select(-c("role", "srctmp", "destmp")) %>%
  filter(!is.na(type)) %>%
  group_by_at(vars(-N)) %>%
  summarise(value = sum(N), .groups = "drop") %>%
  group_by(traj, type, partner, event) %>%
  arrange(traj, time, type) %>%
  mutate(cumvalue = cumsum(value))

mig_cum = df_cases %>%
  filter(event %in% c("IM", "OM")) %>%
  group_by(traj, event, time, age, type) %>%
  summarise(value1 = sum(value), .groups = "drop") %>%
  group_by(traj, event, type) %>%
  arrange(traj, time, type) %>%
  mutate(cumvalue1 = cumsum(value1),
            partner1 = "all") 
            
inout_pop = df_cases %>% 
  mutate(var = ifelse(event %in% c("O", "B", "IM"), "in_pop", "out_pop")) %>%
  group_by(traj, type, var, time, age) %>%
  summarise(value = sum(value), .groups = "drop") %>%
  group_by(traj, type, var) %>%
  arrange(traj, time, type) %>%
  mutate(cumvalue = cumsum(value)) 
...
ge %>%
    # 1. Prepare data
    filter((dest == deme & event == "B") | (dest == deme & event == "M")) %>%
  filter(traj == 1) %>%
    mutate(traj = as.character(traj)) %>%
    bind_rows(case_data %>%
                rename(dest = deme,
