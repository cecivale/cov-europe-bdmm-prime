...
first_eui <- first_df1 %>% 
  ggplot(aes(date)) +
  geom_density(aes(y = ..count..), fill = "#21908CFF", alpha = .5, col = NA) + 
  geom_histogram(binwidth = 1, alpha = .5, color = "grey60") +
  geom_vline(aes(xintercept = median(first_df1$date)), linetype = 1) +
  geom_vline(aes(xintercept = min((first_ecdc %>% filter(deme != "China"))$date)), linetype = 2) +
...
first_euii <- first_df1 %>% 
  ggplot(aes(date)) +
  geom_density(fill = "#21908CFF", alpha = .5, col = NA) + 
  geom_histogram(binwidth = 1, alpha = .5, color = "grey60", aes( y = ..density..)) +
  geom_vline(aes(xintercept = median(first_df1$date)), linetype = 1) +
  geom_vline(aes(xintercept = min((first_ecdc %>% filter(deme != "China"))$date)), linetype = 2) +
...
ggexport(first_eui,
         filename = paste0(args$output_figure, "03i.png"),
         width = 2100, height = 600, res = 300)
ggexport(first_euii,
         filename = paste0(args$output_figure, "03ii.png"),
         width = 2100, height = 600, res = 300)
```
...
first_countryplot <- ggplot(first_country, aes(x = date, y = ..count../(m*args$n))) +
  ...
  ylab("probability density by destination country") +
  xlab("date") +
  labs(title = "First imported case into the country, \nsource") +
  theme(#legend.position = "none",
...
first_10countryplot <- ggplot(first_10country, aes(x = date, y = ..count../(m*args$n))) +
  ...
  ylab("probability density by destination country") +
  xlab("date") +
  labs(title = "First 10 imported cases into the country, \nsource") +
  theme(#legend.position = "none",
        legend.key.size = unit(0.5, "cm"),
        axis.title.x=element_blank()) +
  guides(fill = guide_legend(nrow = 1))
