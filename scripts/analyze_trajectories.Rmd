...
subtitle: "Analysis Europe10 300 particles"
...
# #params$input = "./results/europeC/201211_europe6p2.TL.traj"
# params$input = "/Users/maceci/210205_europe10/results/europeC/210205_europe10.10000.TL.traj"
...
demes <- data.frame(deme = NA, region = NA, country = NA, division = NA,
  ...
  bind_rows(bind_rows(lapply(yaml::yaml.load(string = paste(params$demes, collapse = " ")),
...
df <- loadTrajectories(params$input, burninFrac = params$burnin, subsample = params$n)
...
gribbon <- ggplot(traj_summary %>% filter(var == "active_pop")) +
  ...
  scale_y_log10(labels = trans_format("log10", math_format(10^.x))) +
  scale_x_date(limits = c(ymd("2019-10-01"), ymd(MRS)), date_breaks = "1 month", date_labels = "%b %d") +
...
ggexport(gribbon,
         filename = paste0(params$output_figure, "01.png"),
...
df_traj_cases%>%
  ...
  scale_linetype_manual(name = "", values = c(1, 5),
                        labels = c(paste0(d, " inferred\n population trajectories"),
                                   paste0(ifelse(d == china_deme, "China", as.character(d)),
                                          " total reported cases"))) +
  scale_size_manual(name = "", values = c(0.5, 1.5),
                    labels = c(paste0(d, " inferred\n population trajectories"),
                               paste0(ifelse(d == china_deme, "China", as.character(d)),
                                      " total reported cases"))) +
  scale_x_date(limits = c(ymd("2019-10-01"), ymd(MRS)), date_breaks = "1 month",
               date_labels = "%b %y") +
...
ggexport(ggarrange(plotlist = trajs, labels = chartr("123456789", "ABCDEFGHI", 1:length(trajs)), nrow = 3, ncol = 2),
         filename = paste0(params$output_figure, "02.png"),
...
first_times <- df_traj_cases %>%
  ...
  # 2. Plot
  ggplot() +
  geom_density_ridges(aes(x = date, y = deme, fill = deme), quantile_lines = TRUE, quantiles = c(0.025, 0.975), bandwidth=2, alpha = 0.7, size = 0.2) +
  ...
  geom_point(data = first_ecdc, 
             aes(date, deme, fill = deme), shape = 23, size = 4) +
  scale_fill_manual(values = c(dcolors, "Europe" = "grey")) +
  scale_color_manual(values = c(dcolors, "Europe" = "grey")) +
  scale_x_date(limits = c(ymd("2019-09-01"), ymd("2020-03-08")), date_breaks = "1 month", date_labels = "%b %Y", expand = c(0,0)) +
...
ggexport(first_times,
         filename = paste0(params$output_figure, "03.png"),
         width = 2000, height = 800, res = 300)
```

## First introduction into Europe, location
```{r first-eu, fig.height=4}
# 2.2b First introduction into Europe, distribution density destination
m = 1
first_df1 <- df_traj %>%
  # 1. Prepare data
...
first_eu <- ggplot(first_df1, aes(x = date, y = ..count../(m*params$n))) +
  geom_density_line(
  ...
  ylab("Destination country\nprobability") +
  labs(title = "First imported case into Europe") +
  theme(legend.position = "none",
        axis.title.x=element_blank(),
        axis.text.x = element_text(angle = 90))
first_eu
ggexport(first_eu,
         filename = paste0(params$output_figure, "04.png"),
         width = 2100, height = 600, res = 300)
```

## First 10 introductions into Europe, location
```{r first-eu10, fig.height=4}
# 2.2b First 10 introduction into Europe, distribution density destination
m = 10
first_df10 <- df_traj %>%
  # 1. Prepare data
  filter(var == "IM", !is.na(partner),  value != 0) %>%
...
first_dest <- ggplot(first_df10, aes(x = date, y = ..count../(m*params$n))) +
  geom_density_line(
  ...
  scale_x_date(date_breaks = "1 week", date_labels = "%b %d", expand = c(0,0)) +
  ylab("Destination country\nprobability") +
  labs(title = "First 10 imported cases into Europe") +
  theme(legend.position = "none",
        axis.title.x=element_blank(),
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank())

first_source <- ggplot(first_df10, aes(x = date, y = ..count../(m*params$n))) +
  geom_density_line(
    data = select(first_df10, -deme), aes(fill = NA),
    color = "transparent", alpha = 0.8, bw = 2
  ) + 
...
first_10eu <- ggarrange(first_dest, first_source, labels = chartr("123456789", "ABCDEFGHI", 1:3), nrow = 2, ncol = 1)
first_10eu
ggexport(first_10eu,
         filename = paste0(params$output_figure, "05.png"),
         width = 2000, height = 1200, res = 300)
```

## First introductions into each country, source
...
first_country <- df_traj %>%
  # 1. Prepare data
  filter(var == "IM", !is.na(partner),  value != 0, deme != "China") %>%
  group_by(traj, deme) %>%
  arrange(date, partner) %>%
  slice(1:m)

first_countryplot <- ggplot(first_country, aes(x = date, y = ..count../(m*params$n))) +
  geom_density_line(
    data = select(first_country, -deme), aes(fill = NA),
    color = "transparent", alpha = 0.8, bw = 3
  ) + 
  geom_density_line(aes(fill = partner), bw = 3, color = "transparent", alpha = 0.6) +
  geom_density_line(aes(fill = partner, color = partner), bw = 3, alpha = 0.6) +
  labs(title = "First imported case into each country") +
  facet_wrap(~deme, nrow = 1) +
  scale_fill_manual(name = "", values = dcolors, na.value = "grey") +
  scale_color_manual(name = "", values = dcolors, na.value = "grey") +
  scale_x_date(date_breaks = "1 week", date_labels = "%b %d", expand = c(0,0)) +
  ylab("Source country\nprobability") +
  theme(legend.position = "none",
        axis.title.x=element_blank(),
        axis.text.x = element_text(angle = 90))
first_countryplot
ggexport(first_countryplot,
         filename = paste0(params$output_figure, "06.png"),
         width = 2000, height = 1200, res = 300)
```

## First 10 introductions into each country, source

```{r first-10bycountry, fig.height=4}
...
first_10countryplot <- ggplot(first_10country, aes(x = date, y = ..count../(m*params$n))) +
  geom_density_line(
  ...
  geom_density_line(aes(fill = partner), bw = 3, color = "transparent", alpha = 0.6) +
  geom_density_line(aes(fill = partner, color = partner), bw = 3, alpha = 0.6) +
  labs(title = "First 10 imported cases into each country") +
  facet_wrap(~deme, nrow = 1) +
  scale_fill_manual(name = "", values = dcolors, na.value = "grey") +
  scale_color_manual(name = "", values = dcolors, na.value = "grey") +
  scale_x_date(date_breaks = "1 week", date_labels = "%b %d", expand = c(0,0)) +
  ...
  theme(legend.position = "none",
        ...
        axis.text.x = element_text(angle = 90))
first_10countryplot
ggexport(first_10countryplot,
         filename = paste0(params$output_figure, "07.png"),
         width = 2000, height = 1200, res = 300)
```
...
ratio_plots <- lapply(demes$deme, function(d){
  hweek %>% left_join(inpop) %>% filter(deme == d) %>%
    ...
    xlab("") +
    ylab(ifelse(d %in% c("China", "Italy"), "local/imported cases\n by week", "")) +
    labs(title = d) +
...
ratiobm
ggexport(ratiobm,
           filename = paste0(params$output_figure, "08.png"),
           width = 2000, height = 1200, res = 300)
```
...
plot_chord <- function(df, min_date, max_date) {
  # df <- df %>%
  #   filter(date > min_date, date <= max_date, var == "IM") %>%
  #   group_by(partner, deme, date) %>%
  #   # Take mean value over all trajectories
  #   summarise(Nmean = mean(value), .groups = "drop_last") %>%
...
png(paste0(params$output_figure, "09a.png"), width = 1000, height = 1000, res = 200)
plot_chord(df_traj, ymd("2019-09-01"), ymd("2020-01-23")) 
...
png(paste0(params$output_figure, "09b.png"), width = 1000, height = 1000, res = 200)
plot_chord(df_traj, ymd("2020-01-23"), ymd("2020-02-28"))
...
png(paste0(params$output_figure, "09c.png"), width = 1000, height = 1000, res = 200)
plot_chord(df_traj, ymd("2020-02-28"), ymd("2020-03-08"))
...
imflights
ggexport(imflights,
         filename = paste0(params$output_figure, "10.png"),
         width = 2000, height = 1200, res = 300)
```
...
ggexport(report_plot,
         filename = paste0(params$output_figure, "11.png"),
         width = 2000, height = 1200, res = 300)
```
...
ggexport(inc_plot,
         filename = paste0(params$output_figure, "12.png"),
         width = 2000, height = 1200, res = 300)
```
...
events_timingplot <- ggplot(df_eventsttiming) +
  ...
  scale_linetype_manual(values = 3, name = "") +
  facet_wrap(~deme) +
  scale_fill_viridis_d(guide = FALSE) +
  scale_y_discrete(breaks = c("B", "D", "IM", "OM"), labels = c("first local case", "first uninfectious", "first imported case", "first exported case")) + 
  ylab("") +
  xlab("") +
...
ggexport(events_timingplot,
         filename = paste0(params$output_figure, "13.png"),
         width = 2000, height = 1200, res = 300)
