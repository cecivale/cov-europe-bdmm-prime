...
df <- loadTrajectories2(args$args, burninFrac = args$burnin, subsample = args$n)

# Data wrangling ---------------------------------------------------------------
df_traj <- processEvents(df, demes, "China", mrs)
```
...
knitr::kable((df_eventsttiming %>% 
  left_join(first_ecdc %>% rename(ecdc_date = date)) %>%
  group_by(var, deme) %>%
  filter(date < ecdc_date) %>%
  count() %>%
  mutate(p = n/args$n))[1:12,])
```
::::
...
hweek <- df_traj %>%
  ...
  mutate(R = B/IM) %>%
  mutate(R = ifelse(R == Inf, NA, R)) %>%
  select(traj, deme, year_week, R, B, IM) %>%
  arrange(year_week) 
...
for (d in demes$deme) {
  ...
  print(knitr::kable(hsummary %>% filter(deme == d)))
  cat("\n")
  cat("\n")}
```
...
\small
```{r incidence-table}
knitr::kable(df_inc %>% filter(date == max(date)) %>% select(-date))
```
