...
get_flightData <- function(intraUE_data, extraUE_data, year, month) {
  ...
  df1 <- df %>%
    ...
    mutate(YEAR = as.numeric(substr(TIME,1,4)),
           ...
           PARTNER = case_when(
             ...
             PARTNER == "China including Hong Kong" ~ "China",
             TRUE ~ df$PARTNER)) %>%
    ...
    filter(TRA_MEAS == "Passengers carried (departures)" | 
             (TRA_MEAS == "Passengers carried (arrival)" & PARTNER == "China")) %>%
    mutate(PARTNER = ifelse(TRA_MEAS == "Passengers carried (arrival)" & PARTNER == "China", GEO, PARTNER),
           GEO = ifelse(TRA_MEAS == "Passengers carried (arrival)", "China", GEO),
  ...
  m3["OtherEuropean","OtherEuropean"] <- 0
...
flightsSept <- get_flightData(intraUE_data, extraUE_data, year = 2019, month = 9)
flightsOct <- get_flightData(intraUE_data, extraUE_data, year = 2019, month = 10)
flightsNov <- get_flightData(intraUE_data, extraUE_data, year = 2019, month = 10)
