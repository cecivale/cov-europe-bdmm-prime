...
## using functions from BDMM-Prime project from T.Vaughan 
...
events <- events %>%
  mutate(src = factor(src, levels=c(0,1,2,3,4),
                      labels=c("France", "Germany", "Hubei", "Italy", "Other European")),
         dest = factor(dest, levels=c(0,1,2,3,4),
                       labels=c("France", "Germany", "Hubei", "Italy", "Other European")),
         file = as.numeric(factor(file)),
         traj = paste0(file, ".", traj),
         date = date(date_decimal(decimal_date(ymd(MRS)) - age)))
...
gribbon_deme <- gribbon +
  geom_line(data = case_data %>% rename(type = deme), aes(x = date, y = sumcases), linetype = "dashed") +
  facet_wrap("type", strip.position = "right", nrow = 1) +
  labs(title = "", subtitle="--- ECDC case count data")

## Events
# Histogram of births and migrations by day, by deme
...
line_events <- ggplot(ge) +
  ...
  #geom_freqpoly(aes(x = date, y = Imedian, color = src), stat = "identity", binwidth = 10) +
...
gg1 <- annotate_figure(ggarrange(trajs, trajs_deme, gribbon, gribbon_deme, 
                       ...
                       top = text_grob(paste("Analysis ", str_split(INPUT.TXT[1], pattern = "\\.")[[1]][1]), 
                                       face = "bold", size = 16))
