...
trajs <- states %>% 
  ...
  scale_y_log10(labels = trans_format("log10", math_format(10^.x))) +
  labs(subtitle = paste0("Case trajectories ", deme)) +
...
first_hist <- events %>%
  ...
  labs(subtitle = paste0("First introduction into ", deme, " coloured by source")) +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank())
...
migbirths_box <- ge %>% 
  ...
  labs(subtitle = paste0("Births and migrations events ", deme)) +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank())

# 2.4 Proportion barplot birth vs migration single deme
migbirths_bar <- ge_summary %>%
  ...
  labs(subtitle = paste0("Mean Births vs migrations events percentage ", deme)) +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank())

# 2.5 Proportion barplot source of migrations single deme
srcmig_bar <- ge_summary %>%
  filter(dest == deme, event == "M") %>%
  ...
  scale_x_date(limits = c(ymd("2019-11-01"), ymd("2020-03-08")))+
  scale_fill_manual(values = dcolors) +
  labs(subtitle = paste0("Source of migrations into ", deme, " percentage")) +
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank())
...
dest_bar <- ge_summary %>%
  ...
  scale_x_date(limits = c(ymd("2019-11-01"), ymd("2020-03-08")))+
  scale_fill_manual(values = dcolors)  +
  labs(subtitle = paste0("Destination of migrations from ", deme, " percentage")) 
...
gg2 <- annotate_figure(ggarrange(sample_hist, trajs, first_hist, 
...
gg3 <- annotate_figure(ggarrange(migbirths_box, migbirths_bar,
                                 srcmig_bar, dest_bar, 
